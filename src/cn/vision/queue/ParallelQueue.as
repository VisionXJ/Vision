	...
	public class ParallelQueue extends Queue
	{
		...
		override public function delay($command:Command):Boolean
		{
			if ($command)
			{
				...
				if (result) ArrayUtil.move(commandsIdle, index, commandsIdle.length - 1);
			}
			return result;
		}
		...
		override public function destroy():void
		{
			...
			executor.removeEventListener(CommandEvent.COMMAND_END, executeEndHandler);
			executor.removeEventListener(CommandEvent.COMMAND_START, executeStartHandler);
			executor.destroy();
			executor = null;
		...
		override public function execute($command:Command = null):void
		{
			...
			if ($command)
				(FUNC[$command.priority] ? FUNC[$command.priority] : callNormal) ($command);
			else
				executeCommand();
		}
		...
		{
			if ($command) (FUNC[$command.priority] ? FUNC[$command.priority] : callNormal)($command, false);
		}
		...
		override public function shift($command:Command, $close:Boolean = false):Boolean
		{
			if(!$command.executing)
			{
				...
				if (result)
				{
					ArrayUtil.move(commandsIdle, index, 0);
					if ($close)
					{
						for each (var command:Command in executor.commands)
						{
							command.close();
							commandsIdle.splice(1, 0, command);
							break;
						}
					}
				}
